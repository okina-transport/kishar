        ...
        if (pubsubEnabled) {
            ...
            from ("direct:parse.siri.to.gtfs.rt.trip.updates")
                    .process( p -> {
                        ...
                        Map<String, GtfsRtData> body = siriToGtfsRealtimeService.convertSiriEtToGtfsRt(siri, p.getIn().getHeader("codespaceId").toString().toUpperCase());
            ...
            from ("direct:parse.siri.to.gtfs.rt.alerts")
                    .process( p -> {
                        final byte[] data = (byte[]) p.getIn().getBody();
                        final SiriType siri = SiriType.parseFrom(data);
                        Map<String, GtfsRtData> body = siriToGtfsRealtimeService.convertSiriSxToGtfsRt(siri, p.getIn().getHeader("codespaceId").toString().toUpperCase());
                        p.getOut().setBody(body);
                        p.getOut().setHeaders(p.getIn().getHeaders());
                    })
            ;
            ...
            from ("direct:parse.siri.to.gtfs.rt.stop.monitoring")
                    .process( p -> {
                        final byte[] data = (byte[]) p.getIn().getBody();
                        final SiriType siri = SiriType.parseFrom(data);
                        Map<String, GtfsRtData> body = siriToGtfsRealtimeService.convertSiriSmToGtfsRt(siri, p.getIn().getHeader("codespaceId").toString().toUpperCase());
                        p.getOut().setBody(body);
                        p.getOut().setHeaders(p.getIn().getHeaders());
                    })
            ;
